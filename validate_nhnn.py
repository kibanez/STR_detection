...
def read_validation_tsv(tsv_file, hash_dna2lp):
    ...
    # Convert DNA-ids to string, since python automatically takes as int
    l_dna = map(str, l_dna)
...
def read_identification_tsv(dna2lp_file):
    ...
    # remove all nan , python considers all them as float
    # whaterver we are removing from l_dna we need to do the same in l_lp
    index_nan = np.where(np.isnan(l_dna))

    index_nan = np.array(index_nan)
    index_nan = index_nan.flatten().tolist()

    # we remove here the index positions where we find a nan number (error when trying to casting them to int and then to str)

    for i in sorted(index_nan, reverse=True):
        del l_dna[i]
        del l_lp[i]

    #l_dna = l_dna[-index_nan]
    #l_lp = l_lp[-index_nan]

    # convert to string and before to int, to take off the decimal value
    l_dna = map(int, l_dna)
    l_dna = map(str, l_dna)
